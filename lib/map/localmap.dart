...
class SMap extends StatefulWidget {
  ...
  const SMap({Key? key, required this.initialLocation}) : super(key: key);
...
class _MyAppState extends State<SMap> {
  late GoogleMapController mapController;
  LatLng _center = LatLng(37.5665, 126.9780); // 서울의 좌표로 초기화
  double _zoomLevel = 13.0; // 초기 줌 레벨을 33배로 설정
  ...
  void initState() {
    super.initState();
    _center = widget.initialLocation; // 초기 위치 설정
    _addMarkers();
  }
      ...
      try {
        ...
        mapController.animateCamera(
          CameraUpdate.newCameraPosition(
            CameraPosition(
              target: myLocation,
              zoom: _zoomLevel, // 서울에서 33배 가까이로 줌
            ),
          ),
        );
